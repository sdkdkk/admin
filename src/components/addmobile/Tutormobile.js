...
const Tutormobile = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading1(true);
      const response = await axios.post(`${url}/admin/gettutormobileno`, {
        token: token,
      });
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const requestUrl = data._id
        ? `${url}/admin/tutormobileno`
        : `${url}/admin/tutormobileno`;
      var response;
      response = await axios.post(requestUrl, {
        mobileNo: data.mobileNo,
        contactmobileNo: data.contactmobileNo,
        footermobileNo: data.footermobileNo,
        token: token,
      });
  ...
  function handleDelet(data) {
    axios
      .post(`${url}/admin/tutormobileno/${data}`, {
        token: token,
      })
      .then((response) => {
        fetchData();
          ...
          <div className="main-panel">
            <div className="content-wrapper">
              <div className="page-header">
                <h3 className="page-title">Tutor Mobile No</h3>
              </div>
                                ...
                                <div className="d-flex flex-row-reverse">
                                  <div className="flex-grow-1">
                                    <Controller
                                      ...
                                      rules={{
                                        required:
                                          " Navbar Mobile number is required",
                                        pattern: {
                                          message: "Invalid phone number",
                                        },
                                      }}
                                    ...
                                    {errors.contactmobileNo && (
                                      <p className="error-msg text-danger">
                                        {errors.contactmobileNo.message}
                                      </p>
                                    )}
                                  </div>
                                </div>
                                ...
                                <div className="d-flex flex-row-reverse">
                                  <div className="flex-grow-1">
                                    <Controller
                                      ...
                                      rules={{
                                        required:
                                          "Footer Mobile number is required",
                                        pattern: {
                                          message: "Invalid phone number",
                                        },
                                      }}
                                    ...
                                    />
                                    {errors.footermobileNo && (
                                      <p className="error-msg text-danger">
                                        {errors.footermobileNo.message}
                                      </p>
                                    )}
                                  </div>
                                </div>
                      ...
                      <div className="table-container">
                        <Table
                          ...
                          <thead>
                            <tr>
                              <th>Sr.No</th>
                              <th>Navbar Mobile No</th>
                              <th>Contact Mobile No</th>
                              <th>Footer Mobile No</th>
                              <th>Action</th>
                            </tr>
                          </thead>
                          {loading1 ? (
                            ...
                            <tbody>
                              {displayUsers?.map((data, index, _id) => (
                                <tr key={index}>
                                  ...
                                  <td>
                                    {formatPhoneNumber(data.contactmobileNo)}
                                  </td>
                                  <td>
